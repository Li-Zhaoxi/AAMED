...
%% Read the image name list that no ellipse is detected.

fid = fopen('nodetected.txt','r');
imgnum_nodetected = 0;
imgname_nodetected = [];
while feof(fid) == 0
    imgnum_nodetected = imgnum_nodetected + 1;
    imgname_nodetected{imgnum_nodetected} = fgetl(fid);
end
fclose(fid);
...
%     isValid = 1;
%     for j = 1:imgnum_nodetected
%         if strcmp(imgname_nodetected{j}, imgname{i})
%             isValid = 0;
%             break;
%         end
%     end
